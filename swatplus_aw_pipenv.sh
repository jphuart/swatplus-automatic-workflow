...
# set ERRORLEVEL=0
export PIPENV_PIPFILE=$HOME/.SWAT/SWATPlus/Workflow/Pipfile
...
# export WF_QGIS=/usr/lib/python3/dist-packages:/usr/share/qgis/python/plugins
...
pipenv run python $python_script_prepare_qswat $BASE_DIR
if [ "$?" -ne "0" ]; 
then
  ...
  exit 1
    ...
    pipenv run python $python_script_make_config $BASE_DIR 
    if [ "$?" -ne "0" ]; 
    then
        ...
        pipenv run python $python_script_run_qswat $BASE_DIR
        if [ "$?" -ne "0" ]; 
        then
            ...
            echo 'run_qswat done'
            pipenv run python $python_script_run_editor $BASE_DIR
            echo 'run_editor done'

            pipenv run python $python_script_run_calibration $BASE_DIR
            echo 'un_calibration done'

            pipenv run python $python_script_make_figures $BASE_DIR
...
pipenv run python $python_script_clean_up $BASE_DIR
echo 'clean_up done'
